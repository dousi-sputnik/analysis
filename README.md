...
# ABC分析表の用語の説明
 ...
 累計売上割合 : 累計売上高が実際の売上高の足し算であるのに対して、割合では総売上のうち何%占めているかを % で表します。
 分類　: 優先度を A B Cでランク分けしています。当アプリではAが70%以下、Bが90%以下、CがAとB以下という風に分析結果が出されます。しかしあくまでABC分析の割合は環境や業種によって若干左右されるので +-10%の差で、A B Cの基準がブレます。なので売場や担当商品の種類に応じて累計売上割合を参考に必要なら修正をお願いします。
