...
# ABC分析表の用語の説明
 1. 売上高　 : その商品の売上高を示しています。
    
 2. 累計売上高 : 分析表に入力された全ての商品を上から順番に足し算した数が表示されます。一番下に表示される数はこの分析表の合計の総売上高を表します。これにより「どの商品が総売上高の何千何万何億円占めている」かが可視化されます
   
 3. 累計売上割合 : 累計売上高が実際の売上高の足し算であるのに対して、割合では総売上のうち何%占めているかを % で表します。
   
 4. 分類　: 優先度を A B Cでランク分けしています。当アプリではAが70%以下、Bが90%以下、CがAとB以下という風に分析結果が出されます。しかしあくまでABC分析の割合は環境や業種によって若干左右されるので +-10%の差で、A B Cの基準がブレます。なので売場や担当商品の種類に応じて累計売上割合を参考に必要なら修正をお願いします。
